...
write.csv(dispersal_total_complete, "data/data_process/dispersal_total_complete.csv", row.names = FALSE)

# Dispersal total: Delete species with NA in some traits
# Dispersal total complete: All species even with NA in some traits

#### Include model predictions in the plots
model_average_weibull <- partial_models$model[[1]]

# Check factor levels in the model (training data)
levels(model_average_weibull$data$label)

# Check factor levels in the new data
levels(dispersal_total_complete$label)
...
# Generate predictions from the fitted brms model

ff <- predict(model_average_weibull, 
              newdata = dispersal_total_complete, 
...
ggplot(data= dispersal_total_complete %>% filter(age== "average"), aes(x = PC1, y= Weibull_median)) +
  geom_smooth(aes(fill = age, color = age),
              method = "glm",
              method.args = list(family = gaussian(link = "log")),  # Ensures predictions remain positive
              alpha = 1/4, size = 1/2) +
  #geom_smooth(aes(fill = age, color = age),
   #           method = lm,
    #          alpha = 0.2, size = 0.75) +
  ...
  #coord_cartesian(ylim = c(0, max(dispersal_total_complete$Weibull_median))) +  # Prevents lines below 0
...
########################
 ...
 ggplot(data= dispersal_analysis_partial_age, aes(x = body_mass)) +
   ...
   ggplot(data=dispersal_total_complete %>% filter(age == "average"), 
          aes(x = PC1)) +
     # Plot for median dispersal with custom color
     geom_smooth(aes(y = Weibull_median_log, fill = "Median Dispersal", color = "Median Dispersal"),
                 method = "glm", 
     ...
     # Plot for long-distance dispersal with custom color
     geom_smooth(aes(y = Weibull_upper_distance_log, fill = "Long-Distance Dispersal", color = "Long-Distance Dispersal"),
                 method = "glm", 
                 method.args = list(family = gaussian(link = "log")),
                 alpha = 1/4, size = 1/2, linetype = "dashed") + 
     geom_point(aes(y = Weibull_upper_distance_log, color = "Long-Distance Dispersal"),
                size = 1.3, alpha = 0.6, shape = 17) +
     
     # Use custom color palette
     scale_fill_manual(values = c("Median Dispersal" = "#ff7f0e",  # Blue for median dispersal
                                  "Long-Distance Dispersal" = "#1f77b4")) +  # Orange for long-distance dispersal
     ...
     theme(axis.text = element_text(size = 12), 
           axis.ticks = element_line(linewidth = 1),
           axis.ticks.length = unit(.20, "cm")) +
     labs(title = "Dispersal Analysis (Median and Long-Distance)", 
          color = "Dispersal Type", fill = "Dispersal Type")

   
   ################
   
   
  pc1 <-  ggplot(data=dispersal_total_complete %>% filter(age == "average"), 
          aes(x = PC1)) +
     # Plot for median dispersal with custom color
     geom_smooth(aes(y = Weibull_median_log, fill = "Median Dispersal", color = "Median Dispersal"),
                 method = "glm", 
   ...
   ggsave(pc1, filename = "./results/weibull/figures/scatterplots/PC1_median_long.tiff", width = 8, height = 5, device='tiff', dpi=700)
   
   
   log_HWI <- ggplot(data=dispersal_total_complete %>% filter(age == "average"), 
                      aes(x = log_HWI)) +
     # Plot for median dispersal with custom color
     geom_smooth(aes(y = Weibull_median_log, fill = "Median Dispersal", color = "Median Dispersal"),
                 method = "glm", 
   ...
   ggsave(log_HWI, filename = "./results/weibull/figures/scatterplots/HWI_medina_long.tiff", width = 8, height = 5, device='tiff', dpi=700)
   
   
   
   body_mass <- ggplot(data=dispersal_total_complete %>% filter(age == "average"), 
                       aes(x = body_mass)) +
     # Plot for median dispersal with custom color
     geom_smooth(aes(y = Weibull_median_log, fill = "Median Dispersal", color = "Median Dispersal"),
                 method = "glm", 
   ...
   ggsave(body_mass, filename = "./results/weibull/figures/scatterplots/body_mass_median_long.tiff", width = 8, height = 5, device='tiff', dpi=700)
   
   
   library(patchwork)

   all_plots <- pc1 + log_HWI + body_mass   
   ggsave(all_plots, filename = "./results/weibull/figures/scatterplots/all_median_long.tiff", width = 16, height = 5, device='tiff', dpi=700)
