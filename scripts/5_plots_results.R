...
model_average_weibull <- results_total$model[[1]]

model_breeding_weibull <- results_total$model[[3]]

model_natal_weibull <- results_total$model[[5]]
...
data_model_breeding_weibull <- model_breeding_weibull %>%
  spread_draws(b_log_body_mass, b_diet, b_PC1, b_habita_for, `b_log_body_mass:PC1`, `b_log_body_mass:habita_for`) %>%
  summarise_draws() %>% 
  mutate(age= "breeding", function_id = "weibull")

data_model_natal_weibull <- model_natal_weibull %>%
  spread_draws(b_log_body_mass, b_diet, b_PC1, b_log_HWI, b_habita_for,  `b_log_body_mass:PC1`) %>%
  summarise_draws() %>% 
...
data_models$variable <- 
  recode_factor(data_models$variable, "b_PC1"= "Life history", "b_Latitude"= "Latitude", "b_log_body_mass"= "Body mass",
                ...
                "b_PC1:log_body_mass"= "Body mass : PC1", "b_log_body_mass:diet"= "Body mass : Diet", "b_log_body_mass:habita_for"= "Body mass : Habitat")

median_models <- data_models %>% 
  mutate(descriptor= "median")

write.csv2(data_models, "results/weibull/data_median_models.csv")
