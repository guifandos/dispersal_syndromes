...
predictions <- as.data.frame(predictions)
v_res <- train$Weibull_median_log
model_test <- lm(v_res ~ predictions[, 1])
ff <- model_performance(model_test)
sp_table_total <- tibble(model = list(m), validation = ff, order = "Anseriformes")
...
### Join simple and complex predictions

prediction_complex <- readRDS("Exports/results/prediction_between_within_clade_complex_Weibull.rds")
prediction_complex$complexity <- "total"

prediction_simple_b <- readRDS("Exports/results/prediction_between_within_clade_body_mass_weibull.rds")
prediction_simple_h <- readRDS("Exports/results/prediction_between_within_clade_HWI_Weibull.rds")

names(prediction_complex)
names(prediction_simple_b)


prediction_total <- bind_rows(prediction_complex, prediction_simple_b, prediction_simple_h)
...
ggplot(prediction_total, aes(validation$RMSE, type, complexity)) +
  geom_boxplot(aes(fill = complexity)) +
  theme_classic()
