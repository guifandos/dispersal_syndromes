...
#write.csv2(dispersal_analysis, "dispersal_analysis_natal.csv")

# Best model average weibull median

#load("./revision_analysis/results/weibull/complete_model_weibull_review.RData")
#results_total
#best_weibull_average_median_model <- results_total_complete$model[[5]]$formula
#best_weibull_average_median_model <- "Weibull_median ~ log_body_mass + diet + PC1 + log_body_mass:PC1 + distance_mig + log_HWI + Latitude + (1 | gr(label, cov = A))"

best_weibull_average_median_model <- "Weibull_median ~ log_HWI + log_body_mass + habita_for + PC1 + diet + distance_mig + Latitude + log_body_mass:habita_for + log_body_mass:PC1 + log_body_mass:diet + (1 | gr(label, cov = A))"
####### WITHIN-CLADE #######
...
saveRDS(prediction_total, "prediction_between_within_clade_complex_Weibull_natal_interactions_test_sensitivity.rds")
