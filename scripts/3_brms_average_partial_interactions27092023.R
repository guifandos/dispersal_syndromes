...
st <- c("HWI", "body_mass", "habita_for", "PC1", "diet", "distance_mig", "Latitude", "body_mass:habita_for", "body_mass:PC1", "body_mass:diet")
...
results_average_weibull <- tibble(model = list(model_dispersal_f, model_long_dispersal_f), variable_selection= list(vs1, vs_long), type = "average", function_t= "best_model" )
save(results_average_weibull, file = "Exports/results/model_average_weibull_interactions.RData")

plot_model(model_dispersal_f)
plot_models(model_dispersal_f, model_long_dispersal_f,
                           m.labels = c("median", "long"),
                           show.values = FALSE, show.p = FALSE, p.shape = FALSE) +
  theme_classic() 



data_model_weibull <- model_dispersal_f %>% 
  spread_draws(b_body_mass, b_diet, b_habita_for, `b_PC1:body_mass`, b_Latitude, b_distance_mig, `b_body_mass:diet`, b_PC1) %>%
  summarise_draws() %>% 
  mutate(age= "average", function_id = "weibull")



data_models <-data_model_weibull
...
data_models$variable <- 
  recode_factor(data_models$variable, "b_PC1"= "Life history", "b_Latitude"= "Latitude", "b_body_mass"= "Body mass",
                "b_diet"= "Diet", "b_habita_for"= "Habitat", "b_HWI"= "HWI", "b_distance_mig"= "Distance migration")
