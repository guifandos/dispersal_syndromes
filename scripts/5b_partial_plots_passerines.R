...
load("./revision_analysis/results/weibull/best_model_weibull_review_passerines.Rdata")
...
data_model_average_weibull <- model_average_weibull %>%
  spread_draws(b_PC1, b_log_body_mass, b_diet, b_habita_for, b_distance_mig, `b_log_body_mass:diet`, `b_log_body_mass:PC1`,  b_log_HWI) %>%
  summarise_draws() %>% 
...
data_model_breeding_weibull <- model_breeding_weibull %>%
  spread_draws(b_PC1, b_log_body_mass, b_habita_for, b_distance_mig, `b_PC1:log_body_mass`) %>%
  summarise_draws() %>% 
...
data_models$variable <- 
  recode_factor(data_models$variable, "b_PC1"= "Life history", "b_Latitude"= "Latitude", "b_log_body_mass"= "Body mass",
                ...
                "b_PC1:body_mass"= "Body mass : PC1", "b_log_body_mass:diet"= "Body mass : Diet", "b_log_body_mass:habita_for"= "Body mass : Habitat")

median_models <- data_models %>% 
  mutate(descriptor= "median")


write.csv2(data_models, "./revision_analysis/results/weibull/weibull_models_median_passerines.csv")
...
ggsave("./revision_analysis/results/weibull/figures/median_weibull_passerines.png", plot = median_weibull, width = 8, height = 6)
...
data_model_average_weibull <- model_average_weibull %>%
  spread_draws(b_Latitude, b_log_body_mass, b_diet, b_habita_for, `b_log_body_mass:habita_for`, `b_log_body_mass:diet`) %>%
  summarise_draws() %>% 
  mutate(age= "average", function_id = "weibull")


data_model_breeding_weibull <- model_breeding_weibull %>%
  spread_draws(b_PC1, b_log_body_mass, b_diet, b_habita_for, b_Latitude,  `b_log_body_mass:habita_for`, `b_log_body_mass:diet`, `b_log_body_mass:PC1`) %>%
  summarise_draws() %>% 
  mutate(age= "breeding", function_id = "weibull")

data_model_natal_weibull <- model_natal_weibull %>%
  spread_draws(b_PC1, b_log_body_mass, b_diet, b_habita_for, b_distance_mig,  `b_log_body_mass:diet`, `b_log_body_mass:PC1`) %>%
  summarise_draws() %>% 
  mutate(age= "natal", function_id = "weibull")
...
write.csv2(long_models, "./revision_analysis/results/weibull/weibull_models_long_passerines.csv")
...
long_weibull <- ggplot(data_models, aes(y = variable, x = mean, xmin = q5, xmax = q95)) +
  geom_pointinterval(aes(colour= age, fill=age),
                     position = position_dodge(
                       ## control randomness and range of jitter
                       width = 0.5
...
ggsave("./revision_analysis/results/weibull/figures/long_weibull_passerines.png", plot = long_weibull, width = 8, height = 6)

#### BOTH descriptors together
median_models$descriptor <- "median dispersal"
long_models$descriptor <- "long dispersal"
data_models_both <- rbind(median_models, long_models)
...
both_weibull <-  ggplot(data_models_both %>% filter(function_id== "weibull"), aes(y = variable, x = mean, xmin = q5, xmax = q95)) +
  ...
  xlab("Estimates") 
both_weibull
ggsave("./revision_analysis/results/weibull/figures/both_weibull_passerines.png", plot = both_weibull, width = 8, height = 6)
